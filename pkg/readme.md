# Fever Cookies Plugin

## Introduction

This plugin allows you to setup a fully customizable cookie bar, so the user can control which cookies you can, following the best practices of GDPR compliance.

## Features

- Custom labels
- Custom layout
- Add tabs (required|optional) as you go to match your cookie categories necessities
- Add events to your tab optin/optout actions
...
```js
npm install fever-cookies-plugin
```
...
This will create a default cookie bar, with the default values and no events for optin/optout actions.

![Image of default cookie bar](https://drive.socialworld.pt/d/internal/18-cookies_screen01.png)

#### 3.1 Initialization with custom values

To fully initialize the cookie plugin, you must pass your values/events as parameters. The code below shows setup to the default configuration, all the values that can be edited, plus the google analytics event inside analytics optional tab.
...
For example, in google analytics, the `on` event must implement the `_ga`, `_gid` and `_gat` cookies, and the `off` event must destroy those cookies.

It can have more cookie events for each tab (e.g.: hotjar, facebook), just insert the code to set up them inside each event and it will be called properly when needed.

### 5. Methods

#### 5.1 Open Modal

If you need to open the control panel in another place of your website, you can invoke `openModal()` method like the code below:
...
To expire a cookie - this could help you in the `off` event - you can invoke `expireCookie()` method using the proper name of the cookie you want to expire as an argument.

```js
document.querySelector('your-element').addEventListener('click', () => {
    myCookieBarWithOptions.expireCookie('cookie-name-to-expire')
})
```
